    ...
    getMyCourses()
      .then((response) => {
        ...
        if (response.data.enrollments.length < 1) {
          getTopselling().then((data) => setTopSelling(data.data));
        }
      })
        ...
        <>
          {user?.user?.accountType === "instructor" ? (
            <div className="flex flex-row space-x-3">
              <button onClick={(e) => setShowCreateCourse(true)}
                className="flex items-center space-x-2 bg-white border border-gray-100 py-3 px-3 text-gray-500 font-medium rounded">
          ...
          <div className="bg-blue-100/20 border backdrop-blur-2xl rounded text-blue-900 py-5 px-5 mb-10 flex flex-col space-y-5 font-light">
            <div className="flex flex-col space-y-2">
              <h3 className="font-bold ">
                Welcome to <span>I</span>
                <span>Learn</span>
              </h3>
              <p className="text-sm text-slate-400">
                Welcome to DevTray, where your learning journey begins! We're
                here to guide you through the first steps to help you get the
                most out of our platform. Follow these simple instructions to
                get started
              </p>
            </div>
            <div className="flex flex-col md:flex-row items-baseline space-x-5">
              <div className="lg:w-1/4 flex flex-col space-y-2">
                ...
                <p className="text-sm text-slate-400">
                  create your DevTray account. Choose a unique username and a
              ...
              <div className="lg:w-1/4 flex flex-col space-y-2">
                ...
                <p className="text-sm text-slate-400">
                  Once you've created your account, take some time to browser
                  through our extensive range of course categories.
                </p>
              </div>
              <div className="lg:w-1/4 flex flex-col space-y-2">
                <h3 className="font-medium text-sm">
                ...
                <p className="text-sm text-slate-400">
                  Found a course that aligns with your learning objectives?
                  Great!. Enroll in the course to gain access to its content.
                </p>
              </div>
              <div className="lg:w-1/4 flex flex-col space-y-2">
                <h3 className="font-medium text-sm">
                ...
                <p className="text-sm text-slate-400">
                  Once you've enrolled in a course, dive into the course content
                  and explore the lessons, quizzes and resources provided by
                  instructor.
                </p>
              </div>
            </div>
                  ...
                  <div
                    onClick={(e) =>
                      navigate("/dashboard/explore", {
                        state: { exploreCourses: topSelling, selected: course },
                      })
                    }
                    key={"top-seller-" + index}
                    className="bg-white scale-90 md:scale-100 h-[300px] md:max-h-[250px] relative flex md:w-[500px] w-full shrink-0  rounded-md border border-gray-100"
                  >
                    <div className="flex-1 flex aspect-square">
                      <img
                        className="flex-1"
                        src={API_ROUTES + "/" + course.cover}
                        onError={(e) =>
                        (e.target.src =
                    ...
                    <div className="absolute flex justify-between items-baseline bg-black/10 backdrop-blur-sm w-full p-3 bottom-0 left-0">
                        ...
                        <div className="flex items-center space-x-2">
                          <div className="h-8 w-8 rounded-full flex items-center justify-center bg-gray-400">
                            <p className="font-medium text-xs">
                              {course.author.firstName[0] +
                                "" +
                                course.author.lastName[0]}
                            </p>
                          </div>
                      ...
                      <div>
                        <button className="bg-blue-600 text-white px-5 text-xs font-medium  rounded-full py-2">
                          Enroll {numToCurrency.format(course.price)}
                        </button>
                      </div>
                    </div>
                  </div>
        ...
        <div className="h-[100%] flex-1 ">
          <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
            {courses.map((course, i) => (
              <MyCourseCard item={course} key={`${i}-grid-course`} />
            ))}
          </div>
        </div>
      </div>
