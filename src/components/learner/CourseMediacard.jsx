...
export default function CourseMediacard({ data,onPress }) {
  ...
  return (
    <div onClick={onPress} className="flex transition-all max-w-[full] hover:transition-all hover:scale-105 hover:text-blue-700 delay-100 space-x-3 border border-gray-100 rounded bg-gray-50 p-2 cursor-pointer m-2">
      <div className="max-h-20 w-20 shrink-0 bg-white flex items-center justify-center">
        {
        ...
        {
          data.type.toLowerCase() === 'image' && <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" className="w-10 h-10 text-gray-300">
      ...
      <div className="pr-3">
        <h3 className="md:text-sm text-sm font-medium text-gray-700 truncate">{data?.name.split('.')[0]}</h3>
        {
          data.type.toLowerCase() === 'video' &&  <p className="text-sm font-light">00:30:00</p>
        }
       
        {
          data.type.toLowerCase() === 'image' && <p className="text-sm font-light">Image File</p>
        }
      </div>
    </div>
  )
