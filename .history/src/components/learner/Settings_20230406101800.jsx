import React from "react";

export default function Settings() {
  return (
    <div className="flex-1 flex border-t -mx-5 ">
      <div className="w-1/4 border-r bg-white-50 flex flex-col  space-y-8 border-gray-100 px-5">
        <h1 className="text-2xl pt-5 font-bold text-center">Account Settings</h1>
        <div className="w-full flex flex-col space-y-6 p-3">
          <div className="flex items-center justify-between">
            <div >

            </div>

          </div>

        </div>
      </div>
      <div className="w-3/4 border-r bg-white-50 flex flex-col  space-y-8 border-gray-100 px-5"></div>
    </div>
  );
}
